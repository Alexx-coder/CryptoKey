...
WORDLIST = r'c:\Users\User\Downloads\eff_large_wordlist.txt' #Надо скачать файл по адресу "https://www.eff.org/files/2016/07/18/eff_large_wordlist.txt", затем указать путь к файлу (Тут мой путь)
...
class MenuKey:
    ...
    def menu(self):
        ...
        while True:
            ...
            print("9 - AES (128/256)")
            print("10 - UUID")
            print("11 - Пароль из сид-фраз")
            print("12 - Алгоритм XOR")
            print("13 - HEX в текст")
            print("14 - ECC")
            print("15 - Ed25519")
            ...
            choice = input("Введите (0-13): ").strip()
            if choice == "1":
                print("Переход...")
                time.sleep(1)
                self.random_comb()
    ...
    def aes_(self):
     ...
     while True:
        ...
        if choice == "1":
            ...
            size_choice = input("Введите (1-2): ").strip()
            ...
            print("Расшифровать сообщение AES")
            
            try:
                from Crypto.Cipher import AES
                from Crypto.Util.Padding import unpad
                
                print("Введите зашифрованные данные (Base64):")
                encrypted_b64 = input().strip()
                ...
                if len(encrypted_data) < 16:
                    print("Ошибка: данные слишком короткие")
                    continue
                
                iv = encrypted_data[:16]
                ciphertext = encrypted_data[16:]
                
                print("Введите ключ (HEX):")
                key_hex = input().strip()
                
                if len(key_hex) not in [32, 64]:
                    print("Ключ должен быть 32 (AES-128) или 64 (AES-256) hex символа")
                    continue
    ...
    def seed_password(self):
     print("Генерация Пароля из сид-фраз")
     ...
     while True:
        try:
            ...
            words = self.load_words()  # загружаем список слов
            seed = self.generate_seed(words, word_count=6, separator='-')

            print(f"Seed-фраза: {seed}")
            print(f"Энтропия: {self.entropy(6, len(words)):.1f} бит")

            # спросить, хочет ли пользователь ещё
            ...
            print("Файл eff_large_wordlist.txt не найден! Скачайте данный файл на сайте (https://www.eff.org/files/2016/07/18/eff_large_wordlist.txt), затем в переменную ставьте путь файла.")
            break

        except Exception as seed_p:
            print(f"Ошибка: {seed_p}")
            break
    ...
    def generate_seed(self, wordlist, word_count=12, separator=' '):
     chosen = [secrets.choice(wordlist) for _ in range(word_count)]
     return separator.join(chosen)

    def entropy(self, word_count, wordlist_size):
      import math
      return math.log2(wordlist_size) * word_count
    ...
    def shamir(self):
        ...
        while True:
            try:
               print("1 - Разделить секрет")
               print("2 - Восстановить секрет")
               print("3 - Назад")
    ...
    def _shamir_split(self):
         print("Разделение секрета")
    
         secret = input("Введите секрет: ").strip()
         if not secret:
          print("Секрет не введён")
          return
    
         try:
           n = int(input("На сколько частей разбить: "))
           m = int(input("Сколько частей надо для восстановления: "))
           if m > n:
             print("Ошибка: восстановление не может требовать больше частей")
             return
           if n < 2 or m < 2:
             print("Ошибка: минимум 2 части")
             return
         except:
           print("Ошибка ввода")
           return
         ...
         parts = []
         for _ in range(n - 1):
          part = secrets.token_bytes(len(secret_bytes))
          parts.append(part)
    
          last = secret_bytes
          for p in parts:
            xor_result = bytearray()
            for a, b in zip(last, p):
              xor_result.append(a ^ b)
            last = bytes(xor_result)
          parts.append(last)
         ...
         for i, part in enumerate(parts, 1):
          print(f"Часть {i}: {part.hex()}")
    
         save = input("\nСохранить в файл? (да/нет): ").strip().lower()
         if save == "да":
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"shamir_secret_{timestamp}.txt"
            with open(filename, "w", encoding='utf-8') as f:
               f.write(f"# Нужно {m} частей из {n}\n")
               f.write(f"# Оригинальный секрет: {secret}\n\n")
               for i, part in enumerate(parts, 1):
                  f.write(f"Часть {i}: {part.hex()}\n")
            print(f"Сохранено в {filename}")

         print("\nРазделение выполнено!")

    def _shamir_recover(self):
        print("Восстановление секрета")
    
        parts = []
        part_num = 1
    
        while True:
            part = input(f"Часть {part_num} (hex, Enter для завершения): ").strip()
            if not part:
                if len(parts) < 2:
                  print("Нужно минимум 2 части!")
                  continue
                
                break

            try:
               parts.append(bytes.fromhex(part))
               part_num += 1
            except:
               print("Неверный формат hex. Введите hex-строку без пробелов")
    
        if len(parts) < 2:
            print("Ошибка: нужно минимум 2 части")
            return
    
        print(f"\nВосстанавливаю из {len(parts)} частей...")
        time.sleep(1)
        ...
        for p in parts[1:]:
            xor_result = bytearray()
            for a, b in zip(recovered, p):
               xor_result.append(a ^ b)

            recovered = bytes(xor_result)
    
        try:
           secret = recovered.decode('utf-8')
           print(f"\nВОССТАНОВЛЕННЫЙ СЕКРЕТ: {secret}")
        
           copy_choice = input("\nСкопировать секрет? (да/нет): ").strip().lower()
           if copy_choice == "да":
             try:
                import pyperclip
                pyperclip.copy(secret)
                print("Секрет скопирован!")
             except ImportError:
                print("Установите pyperclip: pip install pyperclip")
        except:
            print(f"Восстановленные данные (hex): {recovered.hex()}")
            print("Возможно, это не текст, а бинарные данные")

    def hex2text(self):
        print("HEX в текст")
        print("Введи HEX-строку (без пробелов)")
        ...
        try:
           ...
           print(f"\nHEX: {hex_input}")
           print(f"Длина: {len(byte_data)} байт")
        
           try:
              text = byte_data.decode('utf-8')
              print(f"ТЕКСТ (UTF-8): {text}")
           except:
              print("ТЕКСТ (UTF-8): Не удалось декодировать")
         
        
           try:
              text = byte_data.decode('ascii')
              print(f"ТЕКСТ (ASCII): {text}")
           except:
             pass
        
        
           print(f"БАЙТЫ: {byte_data[:100]}")
        
           copy_choice = input("\nСкопировать текст? (да/нет): ").strip().lower()
           if copy_choice == "да":
             try:
                import pyperclip
                try:
                    pyperclip.copy(byte_data.decode('utf-8'))
                except:
                    pyperclip.copy(hex_input)
                print("Скопировано!")
             except ImportError:
                print("Установите pyperclip: pip install pyperclip")
                
        except Exception as error:
          print(f"Ошибка: {error}. Неверный HEX формат")
    ...
    def _ecc_gen(self):
        try:
          ...
          # Вывод ваших ключей
          print("\n=== ECC Ключи ====")
          print(f"Ваш приватный ключ: {private_pem}")
          print(f"Ваш публичный ключ: {public_pem}")
          copy_choice = input("\nСкопировать публичный ключ? (Да/Нет): ").strip().lower()
          if copy_choice == "да":
            try:
               import pyperclip
    ...
    def _ecc_sign(self):
        try:
            print("Подпись сообщения ECC")

            private_pem = input("Введите ваш приватный ECC ключ: ").strip()
            ...
            print(f"Ваша подпись (Base64): \n{signature_b64}")
            copy_choice = input("\nСкопировать подпись? (Да/Нет): ").strip().lower()
            if copy_choice == "да":
                try:
                    import pyperclip
                    pyperclip.copy(signature_b64)
                except ImportError:
                    print("Установить pyperclip: pip install pyperclip")
            
        except Exception as error:
            print(f"Произошла ошибка: {error}")

    def _ecc_check_sign(self):
        try:
            print("Проверка подписи ECC")

            public_pem = input("Введите публичный ECC ключ : ").strip()
            message = input("Введите исходное сообщение: ").strip()
            signature_b64 = input("Введите подпись (Base64): ").strip()
            ...
            print(f'Подпись неверна: {error}')

    def ed25519_menu(self):
        print("ED25519")
        while True:
            print("1 - Генерация ключей")
            print("2 - Подписать сообщение")
            ...
            if choice == "1":
                ...
                print("Перехожу...")
                time.sleep(1)
                self._ed25519_verify()
            else:
                print("Не распознано...")
                time.sleep(1)
                continue

    def _ed25519_gen(self):
        try:
            print("Генерация ключей Ed25519")
    ...
    def _ed25519_sign(self):
        try:
            print("Подпись сообщения Ed25519")

            private_pem = input("Введите приватный ключ (PEM): ").strip()
            message = input("Введите сообщение для подписи: ").strip()
            ...
            if copy_choice == "да":
                try:
                    import pyperclip
                    pyperclip.copy(signature_b64)
                    print("Подпись скопирована")
                except ImportError:
                    print("Установите pyperclip: pip install pyperclip")
        except Exception as error:
            print(f"Произошла ошибка: {error}")

    def _ed25519_verify(self):
        try:
            print("Проверка подписи Ed25519")

            public_pem = input("Введите публичный ключ (PEM): ").strip()
            message = input("Введите исходное сообщение: ").strip()
            signature_b64 = input("Введите подпись (base64): ").strip()
